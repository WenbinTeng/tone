    ...
    for i in l:
        ...
        if code_dict[decoder.id] in {"add", "sub", "slt", "sltu", "sll", "srl", "sra", "xor", "or", "and", "addi", "slti", "sltiu", "xori", "ori", "andi", "slli", "srli", "srai", "lw", "sw"}:
            inst_list = inst_list + [
                ...
                subleq(end_dict[decoder.id], RZ, pc + 4 * INST_WIDTH),
                subleq(RZ, RZ, start_dict[decoder.id])
            ]
            pc = pc + 5 * INST_WIDTH
        elif code_dict[decoder.id] in {"beq", "blt", "bltu"}:
            if decoder.b_imm < 0x80000000:
                wait_dict[len(addr_list) + decoder.b_imm] = pc + INST_WIDTH
            inst_list = inst_list + [
                subleq(RZ, RZ, pc + 2 * INST_WIDTH),
                [pc + 7 * INST_WIDTH, addr_list[-(~decoder.b_imm&0xffffffff)+1] if decoder.b_imm >= 0x80000000 else 0, 0],
                subleq(RZ, pc + 1 * INST_WIDTH, pc + 3 * INST_WIDTH),
                subleq(end_dict[decoder.id], RZ, pc + 4 * INST_WIDTH),
                subleq(RZ, RZ, pc + 5 * INST_WIDTH),
                subleq(RZ, pc + 1 * INST_WIDTH + WORD_WIDTH, pc + 6 * INST_WIDTH),
                subleq(end_dict[decoder.id] - INST_WIDTH, RZ, pc + 7 * INST_WIDTH)
            ]
            pc = pc + 7 * INST_WIDTH
        elif code_dict[decoder.id] in {"bne", "bge", "bgeu"}:
            if decoder.b_imm < 0x80000000:
                wait_dict[len(addr_list) + decoder.b_imm] = pc + INST_WIDTH
            inst_list = inst_list + [
                subleq(RZ, RZ, pc + 2 * INST_WIDTH),
                [pc + 7 * INST_WIDTH, addr_list[-(~decoder.b_imm&0xffffffff)+1] if decoder.b_imm >= 0x80000000 else 0, 0],
                subleq(RZ, pc + 1 * INST_WIDTH, pc + 3 * INST_WIDTH),
                subleq(end_dict[decoder.id] - INST_WIDTH, RZ, pc + 4 * INST_WIDTH),
                subleq(RZ, RZ, pc + 5 * INST_WIDTH),
                subleq(RZ, pc + 1 * INST_WIDTH + WORD_WIDTH, pc + 6 * INST_WIDTH),
                subleq(end_dict[decoder.id], RZ, pc + 7 * INST_WIDTH)
            ]
            pc = pc + 7 * INST_WIDTH
